...
vec3 checkAndAdjustSphereCollisions(vec4 p1, vec3 v1) {
  ...
  for(float i = 0.0; i < MAX_ITER; i++) {
    ...
    vec3 diff = gp1-gp2;
    float d = distance(gp1, gp2);

    float rSum = p1.w + p2.w;
    if (d < rSum) { // There's a collision!
      v1.x = ( (p1.w - p2.w)*v1.x + 2.0*p2.w*v2.x )/(rSum);
      v1.y = ( (p1.w - p2.w)*v1.y + 2.0*p2.w*v2.y )/(rSum);
      v1.z = ( (p1.w - p2.w)*v1.z + 2.0*p2.w*v2.z )/(rSum);
      
      // v1 = ( (p1.w - p2.w)*v1 + 2.0*p2.w*v2 )/(rSum); 
    }
