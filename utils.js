...
function createProgram(gl, sourceVS, sourceFS) {
  const vertexShader = createShader(gl, sourceVS, gl.VERTEX_SHADER);
  const fragmentShader = createShader(gl, sourceFS, gl.FRAGMENT_SHADER);
...
class RayTracer {
  ...
  initProg(vs, fs) {
    ...
    gl.useProgram(this.prog);
    gl.uniform1i(
      gl.getUniformLocation(this.prog, "bounceLimit"),
      this.bounceLimit,
    );
    for (var i = 0; i < spheres.length; ++i) {
      gl.uniform3fv(
        gl.getUniformLocation(this.prog, "spheres[" + i + "].center"),
        spheres[i].center,
      );
      gl.uniform1f(
        gl.getUniformLocation(this.prog, "spheres[" + i + "].radius"),
        spheres[i].radius,
      );
...
class PrimaryRayTracer extends RayTracer {
  init() {
    this.initProg(
      document.getElementById("raytraceVS").text,
      raytraceFS_primary,
    );
  }
  draw(trans) {
...
class SecondaryRayTracer extends RayTracer {
  ...
  init() {
    this.initProg(
      document.getElementById("sphereVS").text,
      raytraceFS_secondary,
    );
    if (!this.prog) return;
...
var screenQuad = {
  ...
  draw(prog, trans) {
    gl.useProgram(prog);
    gl.uniformMatrix4fv(
      gl.getUniformLocation(prog, "c2w"),
      false,
      trans.camToWorld,
    );
    gl.bindBuffer(gl.ARRAY_BUFFER, this.vbuf);
...
var background = {
  ...
  updateProj() {
    gl.useProgram(this.prog);
    gl.uniformMatrix4fv(
      gl.getUniformLocation(this.prog, "proj"),
      false,
      perspectiveMatrix,
    );
  },
...
function InitEnvironmentMap() {
  ...
  const url =
    "https://webglfundamentals.org/webgl/resources/images/computer-history-museum/";
  const files = [
    "pos-x.jpg",
    "neg-x.jpg",
    "pos-y.jpg",
...
function UseEnvironmentMap(param) {
  gl.bindTexture(
    gl.TEXTURE_CUBE_MAP,
    param.checked ? environmentTexture : null,
  );
  DrawScene();
}
